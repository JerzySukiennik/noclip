...
import { CELL, CAM, PLAYER, RES, GRID_W } from './config.js?v=51398ca1788208342';
import { Level, cxOf, cyOf, wx, wz } from './level.js?v=51398ca1788208342';
import { setAnisotropy } from './tex.js?v=51398ca1788208342';
import { Player } from './player.js?v=51398ca1788208342';
import { Entity, STATE } from './entity.js?v=51398ca1788208342';
import { LightPool } from './lights.js?v=51398ca1788208342';
import { Audio } from './audio.js?v=51398ca1788208342';
import { FX } from './fx.js?v=51398ca1788208342';
import { Hud } from './hud.js?v=51398ca1788208342';
import { Net, tapeCode, newTapeCode, setTapeCode, hashTape } from './net.js?v=51398ca1788208342';
import { createMenu } from './menu.js?v=51398ca1788208342';
import { Menu3D } from './menu3d.js?v=51398ca1788208342';
import { Touch, isTouchDevice } from './touch.js?v=51398ca1788208342';
...
async function boot() {
  ...
  window.__ncStage = 'building level';
  ...
  window.__ncStage = 'threading tape';
  ...
  window.__ncStage = 'fetching audio';
  ...
  document.addEventListener('pointermove', (e) => { if (menu3d) menu3d.setPointer(e.clientX, e.clientY); });
  // A tap has no preceding move, so take the aim from the press itself.
  canvas.addEventListener('pointerdown', (e) => { if (menu3d) menu3d.setPointer(e.clientX, e.clientY); });
  canvas.addEventListener('click', (e) => { if (menu3d) menu3d.click(e.clientX, e.clientY); });
  canvas.addEventListener('touchend', (e) => {
    if (!menu3d) return;
    const t = e.changedTouches && e.changedTouches[0];
    if (!t) return;
    e.preventDefault();
    menu3d.click(t.clientX, t.clientY);
  }, { passive: false });
  ...
  addEventListener('keydown', (e) => {
    ...
    if (e.code === 'Escape') menu.closePanel();
    if (e.code === 'Enter' && !menu.panelOpen()) menu.launch('solo');
  });

  // Every zone type has its own materials, and three compiles a shader program the
  // first time each one is actually drawn - which lands as a multi-hundred-millisecond
  // stall the moment you walk into a new kind of room. Compile the lot up front.
  window.__ncStage = 'warming shaders';
  hud.bootText('warming shaders');
  await frame();
  // A shader program is per material, not per mesh, so one chunk from each zone
  // compiles everything. Showing all ~450 chunks to do it stalls for seconds.
  const shown = [];
  for (const z of level.zones) {
    level.cullChunks(1e6, 1e6, 1);
    const c = z.chunks[0];
    if (!c) continue;
    for (const part of c.parts) { part.visible = true; shown.push(part); }
  }
  camera.position.set(level.spawnWorld.x, 1.6, level.spawnWorld.z);
  entity.root.visible = true;
  player.rig.visible = true;
  try { renderer.compile(scene, camera); } catch (e) { console.info('[noclip] precompile skipped', e); }
  entity.root.visible = false;
  for (const part of shown) part.visible = false;

  window.__ncStage = 'running';
  hud.hideBoot();
  started = true;
  clock.start();
  startHeartbeat();
...
// The scene renders into a 512x384 target, but the VHS composite runs once per
// OUTPUT pixel with barrel warp, chroma taps, grain and dropout - so on a big
// display it, not the scene, is the expensive pass. Cap the backing store and let
// CSS stretch it; on a tape that reads as the format, not as a compromise.
let outCap = 1280;
function onResize() {
  const s = Math.min(1, outCap / Math.max(1, innerWidth));
  const w = Math.max(320, Math.round(innerWidth * s));
  const h = Math.max(240, Math.round(innerHeight * s));
  renderer.setSize(w, h, false);
  if (fx) fx.setOutputSize(w, h);
...
function applySettings(s) {
  ...
  outCap = s.quality === 'low' ? 900 : (s.quality === 'high' ? 1700 : 1280);
  if (isTouch) { q = [Math.round(q[0] * 0.78), Math.round(q[1] * 0.78)]; outCap = Math.min(outCap, 820); }
...
function openMenu() {
  ...
  menu = createMenu({
    ...
    audioState: () => (audio.ctx ? audio.ctx.state : 'suspended'),
    onEnableSound: () => { audio.init(); audio.resume(); audio.kick(); },
    onStart: (route, code) => startRun(route, code)
  });
  // The menu is a board on a wall in the actual level, so the camera stands in
  // front of it and drifts with the cursor instead of hanging over a blurred still.
  try {
    menu3d = new Menu3D(scene, camera, level, {
    items: [
      { label: 'Enter the tape', action: () => menu.launch('solo') },
      { label: 'Play together', action: () => menu.launch('host') },
      { label: 'Join a tape', action: () => menu.openPanel('join') },
      { label: 'Settings', action: () => menu.openPanel('settings') },
      { label: 'Controls', action: () => menu.openPanel('controls') }
      ]
    });
    if (!menu3d.buttons.length) throw new Error('no wall found');
  } catch (e) {
    console.warn('[noclip] wall menu unavailable, using the list', e);
    menu3d = null;
    menu.showFallbackNav();
  }
...
async function startRun(route, code) {
  ...
  inMenu = false;
  if (menu3d) { menu3d.dispose(); menu3d = null; }
  player.rig.visible = true;
  ...
  audio.resume();
  audio.kick();
  // Keep the menu track running through the fall - otherwise it is cut by the very
  // click that starts the game and you never actually hear it.
...
function startHeartbeat() {
  ...
  w.onmessage = () => {
    if (started && document.hidden) {
      heartbeatOwns = true;
      // One render path, no debug-only branch: a flag that made drawing happen
      // only in tests is exactly how a menu that never rendered got signed off.
      advance(Math.min(0.05, clock.getDelta()));
...
function advance(dt) {
  T += dt;
  // The menu used to return before this point, so the scene behind it was never
  // drawn at all - which was invisible while the menu was a DOM overlay, and a
  // black screen the moment the menu became the scene.
  if (inMenu) menuFrame(dt);
  else if (!over) step(dt);
  fx.u('uTime', T);
  fx.u('uHurt', hurt);
  fx.u('uGlitch', glitch);
  fx.u('uBattery', battery);
  fx.u('uFade', fadeK);
  fx.u('uTrack', 0.07 + glitch * 0.30 + (1 - battery) * 0.16);
...
function step(dt) {
  ...
  // hand the menu track over the moment you actually land, not on a timer
  if (handoverPending && !player.falling && player.grounded) {
  ...
  const fwd = vFwd.set(0, 0, -1).applyQuaternion(camera.quaternion);
  lights.setHand(
  ...
  lights.update(level, camera.position.x, camera.position.y, camera.position.z, T);
  cullT -= dt;
  if (cullT <= 0) { cullT = 0.12; level.cullChunks(player.pos.x, player.pos.z, 58); }
  ...
  if (stepT <= 0 && ed < 20) {
    ...
    // irregular, or it turns into a metronome you stop hearing as footsteps
    stepT = (spd > 3 ? 0.30 : 0.74) * (0.86 + Math.random() * 0.3);
    if (spd > 0.4) {
      const rel = vRel.set(entity.pos.x - camera.position.x, 0, entity.pos.z - camera.position.z).normalize();
      const right = vRight.set(1, 0, 0).applyQuaternion(camera.quaternion);
      const pan = Math.max(-1, Math.min(1, rel.dot(right)));
      // no volume floor: at 20 m it has to be almost nothing, or it nags
      const att = Math.pow(Math.max(0, 1 - ed / 20), 2.2);
      audio.play('thud', { vol: 0.34 * att, rate: 0.44 + Math.random() * 0.10, pan, lp: 620 + att * 900 });
    }
  }

  // proximity dread: the tape degrades as it closes in
  const prox = Math.max(0, 1 - ed / 22);
  hurt += ((entity.state >= STATE.STALK ? prox * 0.20 : 0) - hurt) * Math.min(1, dt * 2.2);
  glitch += ((entity.state === STATE.HUNT ? 0.26 + prox * 0.20 : prox * 0.06) - glitch) * Math.min(1, dt * 1.6);
