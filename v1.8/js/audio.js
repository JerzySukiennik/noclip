...
export class Audio {
  ...
  async loadLocalMusic() {
    // manifest.local.json holds music that is not committed, so it is only worth
    // asking for on a dev host - on production it would just be a 404 in the console.
    const local = ['localhost', '127.0.0.1', '[::1]'].includes(location.hostname);
    const sources = local ? ['audio/manifest.local.json', 'audio/manifest.json'] : ['audio/manifest.json'];
    let man = null;
    for (const src of sources) {
      try {
        ...
        if (j && SLOTS.some(k => Array.isArray(j[k]) ? j[k].length : j[k])) { man = j; break; }
        if (!man) man = j;
      } catch (e) { /* fall through to the next source, then to the synth beds */ }
    }
