...
**On a phone** — left thumb walks, and pushing the stick to its rim runs; right thumb
looks; LAMP, DUCK and BACK sit bottom-right. Hold it sideways. Append `?touch=1` (or
`?touch=0`) to force the on-screen pad on or off, which is the only way to test a
landscape phone from a desktop browser.
...
Before deploying, run `python3 tools/stamp.py`. It writes the same kind of version
query into the committed files. GitHub Pages serves the repo directly with a long
cache TTL and no fingerprinting, so without it a browser will happily run new and old
modules side by side after a deploy, which looks like a bug in whatever changed.
...
With no files present the game synthesises its own beds, so it still works everywhere.

The tracks are committed at the repo root only. Version snapshots deliberately drop
`audio/`, because copying ~26 MB into every frozen version would grow the repo by that
much per deploy; an archived version therefore falls back to the synth beds.
...
js/menu.js        front screen over the live level
