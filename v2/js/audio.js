...
export class Audio {
  ...
  _initPlayer() {
    ...
    el.addEventListener('timeupdate', () => {
      ...
      if (!e || !e.end) return;
      const left = e.end - el.currentTime;
      // Duck out of the seam and back in: jumping from a full-bodied bar to a
      // track's near-silent first frame is otherwise an audible cut every loop.
      if (left < 0.7 && left > 0 && !this._looping) {
        this._looping = true;
        const g = this.elGain.gain, n = this.ctx.currentTime;
        g.cancelScheduledValues(n);
        g.setValueAtTime(Math.max(0.0001, g.value), n);
        g.linearRampToValueAtTime(0.0001, n + Math.max(0.12, left));
      }
      if (left <= 0.06) {
        if (this._slotList(this.musicMode).length < 2) {
          try { el.currentTime = e.start; } catch (err) { }
          const g = this.elGain.gain, n = this.ctx.currentTime;
          g.cancelScheduledValues(n);
          g.setValueAtTime(0.0001, n);
          g.linearRampToValueAtTime(this._volFor(this.musicMode), n + 1.1);
          this._looping = false;
