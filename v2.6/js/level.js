...
import { CELL, GRID_W, GRID_H, CHUNK, ROOM_TYPES, TYPE_BY_ID, SCRAWL, NOTES } from './config.js?v=77e8fa21788113364';
import { Rng } from './rng.js?v=77e8fa21788113364';
import { wallTex, floorTex, ceilTex, lampTex, waterTexture, scrawlTexture } from './tex.js?v=77e8fa21788113364';
...
export class Level {
  ...
  // Height of walkable ground under a point. Flat everywhere except the Level 94
  // hills, which are ellipsoid caps you can climb.
  groundAt(x, z) {
    let y = 0;
    for (const h of this.hills) {
      ...
      const top = Math.sqrt(1 - d2 / (h.r * h.r)) * h.r * h.sy - h.sink;
      if (top > y) y = top;
    }
    return y;
  }
  ...
  _buildOutdoors(z) {
    ...
    // Rolling hills. They are ground, not obstacles, so each one also goes into a
    // height field the player walks over - otherwise you stroll straight through.
    for (let k = 0; k < 7; k++) {
      const r = R.range(9, 20);
      const sy = R.range(0.22, 0.40);
      const sink = r * sy * R.range(0.30, 0.60);
      const hx = cxw + R.range(-span * 0.42, span * 0.42);
      const hz = czw + R.range(-span * 0.42, span * 0.42);
      const m = new THREE.Mesh(new THREE.SphereGeometry(r, 18, 12), grassMat);
      m.scale.y = sy;
      m.position.set(hx, -sink, hz);
      g.add(m);
      this.hills.push({ x: hx, z: hz, r, sy, sink, top: r * sy - sink });
    }

    const solid = (px, pz, hw, hd) => this.propBoxes.push({ x: px, z: pz, hw, hd });
    ...
    // the fence blocks in short segments so it can be walked along, not through
    for (let i = 0; i < 9; i++) {
      solid(cxw - span * 0.42 + (i + 0.5) * (span * 0.84 / 9), fz, span * 0.048, 0.10);
    }
    ...
    g.add(tw);
    solid(tw.position.x, tw.position.z, 1.7, 1.7);
