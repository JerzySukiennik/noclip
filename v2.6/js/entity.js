...
import { CELL, GRID_W, GRID_H, ENTITY } from './config.js?v=77e8fa21788113364';
import { cxOf, cyOf, wx, wz } from './level.js?v=77e8fa21788113364';
import { collideCircle } from './player.js?v=77e8fa21788113364';
...
export class Entity {
  ...
  update(dt, t, targets, difficulty = 1) {
    ...
    switch (this.state) {
      case STATE.DORMANT:
        // this close it knows you are there whether it saw or heard you
        if (d < E.noticeDist || heard || (los && d < E.seeDist)) this.setState(STATE.AWARE);
        break;
      case STATE.AWARE:
        if (d < E.freezeMin) this.setState(STATE.HUNT);
        else if (los && d < 22) this.setState(STATE.STALK);
    ...
    // Freezing is a distance behaviour. Close in it must never stand and stare -
    // that was the bug: a dormant Entity a few metres away just stopped forever.
    if (this.observed && this.state !== STATE.HUNT && d > E.freezeMin) speed = 0;

    this.pathT -= dt;
    if (goal && this.pathT <= 0) {
      this.pathT = E.repathEvery * (d > 40 ? 3 : 1);
