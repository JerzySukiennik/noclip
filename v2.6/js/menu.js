...
const CSS = `
.nc-screen{position:fixed;inset:0;z-index:8;pointer-events:none;color:${C.text};
...
.nc-veil{position:absolute;inset:0;background:transparent;pointer-events:none}
.nc-vignette{position:absolute;inset:0;pointer-events:none;
background:radial-gradient(130% 100% at 50% 48%,transparent 52%,rgba(4,3,1,.55) 100%)}
.nc-grain{position:absolute;inset:0;pointer-events:none;opacity:.20;
...
.nc-sound{position:absolute;pointer-events:auto;left:50%;bottom:calc(30px + env(safe-area-inset-bottom));
...
.nc-foot{position:absolute;pointer-events:none;left:clamp(28px,7vw,120px);bottom:34px;display:flex;gap:22px;align-items:center;
...
.nc-panel{grid-area:1/1;position:relative;pointer-events:auto;width:min(430px,44vw);padding:26px 28px 28px;border-radius:14px;
...
export function createMenu(opts) {
  ...
  // Hidden while the wall menu is alive; shown as a fallback if it never appears,
  // so the game can never end up with no way in.
  const nav = el('nav', 'nc-nav', layout);
  nav.style.display = 'none';
  nav.style.pointerEvents = 'auto';
  const panels = el('div', 'nc-panels', layout);
  const foot = el('div', 'nc-foot', root);
  // The build id is on screen on purpose: "still broken" is unanswerable without
  // knowing whether the browser is even running the build that fixed it.
  const src = document.querySelector('script[type="module"][src]');
  const build = (src && (src.getAttribute('src').split('?v=')[1] || '')).slice(0, 12);
  el('div', null, foot, (cfg.footer || 'gzowo.fun') + (build ? '  ·  build ' + build : '  ·  dev'));
  ...
  return {
    ...
    launch,
    openPanel: (name) => {
      const map = { join: joinPanel.node, settings: setPanelUI.node, controls: ctrlPanel.node };
      const node = map[name];
      if (node) { openedFrom = null; setPanel(node); }
    },
    showFallbackNav: () => {
      nav.style.display = '';
