...
import { Net, tapeCode } from './net.js';
...
async function boot() {
  ...
  const seedFromHash = (location.hash.match(/tape=([A-Z0-9]+)/i) || [])[1];
  const seed = seedFromHash ? [...seedFromHash].reduce((a, c) => (a * 31 + c.charCodeAt(0)) >>> 0, 7) : (Math.random() * 1e9) | 0;
...
function onMouse(e) {
  ...
  player.addLook(e.movementX * 0.0022, e.movementY * 0.0022);
}
...
function onNetEvent(kind, val, from) {
  ...
  if (kind === 'hunt' && !net.isHost) { audio.setMusic('chase'); hud.alert('RUN'); }
...
function respawn() {
  ...
  player.respawnAt(wx(cx) + CELL / 2, wz(cy) + CELL / 2);
  entity.reset();
  dead = 0;
  hurt = 0;
...
function step(dt) {
  ...
  if (dead) {
    deadT += dt;
    // the camera is dragged toward it, then the tape cuts
    tmp.set(entity.pos.x, 1.55, entity.pos.z).sub(camera.position);
    const want = Math.atan2(-tmp.x, -tmp.z);
    let dy = want - player.yaw;
    while (dy > Math.PI) dy -= Math.PI * 2;
    while (dy < -Math.PI) dy += Math.PI * 2;
    player.yaw += dy * Math.min(1, dt * 9);
    player.pitch += (0.16 - player.pitch) * Math.min(1, dt * 6);
    player.input.f = player.input.s = 0;
    entity.pos.lerp(new THREE.Vector3(player.pos.x, 0, player.pos.z), Math.min(1, dt * 2.4));
    entity.animate(dt, 0.6, { pos: camera.position });
    player.update(dt, T);
    hurt = Math.min(1, hurt + dt * 0.7);
    fadeK = Math.max(0, 1 - Math.max(0, deadT - 1.1) * 1.6);
    if (deadT > 2.4) { hud.blackout(true); }
    if (deadT > 3.6) { fadeK = 1; respawn(); }
  ...
  // ---- entity ------------------------------------------------------------
  const targets = [{
    pos: player.pos, speed: player.speed, crouching: player.crouching,
    alive: player.alive && !dead, noise
  }];
  for (const [, p] of net.players) targets.push({ pos: p.pos, speed: p.speed, crouching: p.crouch > 0.5, alive: p.alive, noise: p.noise });
  ...
  if (!dead && player.alive && ed < 1.35 && entity.root.visible) kill();
