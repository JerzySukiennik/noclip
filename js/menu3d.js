...
export class Menu3D {
  ...
  click() {
    ...
    if (this.hover && this.hover.item.action) { this.hover.item.action(); return true; }
    return false;
  }
  ...
  update(dt) {
    ...
    const hit = hits.length ? this.buttons.find(b => b.plate === hits[0].object) : null;
    if (hit !== this.hover) {
      this.hover = hit;
