...
export class Audio {
  ...
  async loadLocalMusic() {
    ...
    // The manifest is committed but the audio files are gitignored, so a deployed
    // build routinely lists tracks that are not there. Probe once, up front, rather
    // than discovering it through <audio> error events mid-run.
  ...
  async _pruneMissing(t) {
    ...
    const [op, hunt] = await Promise.all([exists(t.opening), exists(t.hunt)]);
    const keep = await Promise.all(t.explore.map(exists));
    return {
      opening: op ? t.opening : null,
      hunt: hunt ? t.hunt : null,
      explore: t.explore.filter((_, i) => keep[i])
    };
  }
  ...
  _initPlayer() {
    ...
    el.addEventListener('error', () => {
      this.musicFails++;
      // A missing file must never leave the mix silent. Outside the playlist there
      // is no next track to try, so hand straight over to the next sensible mode.
      if (this.musicMode === 'opening') { this.musicMode = null; this.setMusic('explore', true); return; }
      if (this.musicMode === 'chase' || this.musicMode === 'end') { this._noMusic(); return; }
      if (this.musicFails > this.tracks.explore.length) { this._noMusic(); return; }
  ...
  // immediate=true skips the between-track silence: it is used when a file failed
  // to load, where waiting would leave a minute of dead air before the fallback.
  _nextTrack(immediate) {
    if (this.musicMode !== 'explore') return;
    if (!this.queue.length) this.queue = this._shuffledExplore();
    const next = this.queue.shift();
    if (!next) { this._noMusic(); return; }
    clearTimeout(this._gap);
    const gap = immediate ? 0 : 4000 + Math.random() * 9000;
    this._gap = setTimeout(() => {
      if (this.musicMode !== 'explore') return;
      this._playFile(next, 0.42, immediate ? 0.5 : 3.0);
    }, gap);
  }

  // Every track failed to load - fall back to the synth beds for the rest of the run.
  _noMusic() {
    this.hasMusic = false;
    this.tracks = { opening: null, explore: [], hunt: null };
    const m = this.musicMode;
    this.musicMode = null;
    this.setMusic(m === 'chase' ? 'chase' : 'explore', true);
  }

  _playFile(url, vol, fade = 3.0) {
    if (!this.el) return;
    this.el.src = url;
    // After an error the element stays in a failed state until load() is called,
    // so without this the fallback stalls on the first missing file.
    this.el.load();
  ...
  setMusic(mode, immediate) {
    ...
    if (mode === 'opening') {
      if (this.tracks && this.tracks.opening) { this._playFile(this.tracks.opening, 0.5, 1.4); return; }
      this.musicMode = null;
