...
void main(){
  ...
  float shear = (noiseRow - 0.5) * 0.0016 * (0.35 + uTrack * 3.4);
  float sweep = fract(uTime * 0.28);
  float nearSweep = smoothstep(0.055, 0.0, abs(uv.y - sweep));
  shear += nearSweep * (hash(vec2(band, 7.0)) - 0.5) * 0.020 * (0.25 + uTrack);
  ...
  float ca = (0.0014 + r2 * 0.006) * (1.0 + uHurt * 5.0 + uTrack * 2.0);
  vec3 col = vec3(
  ...
  col = mix(col, mix(col, smear, 0.5), 0.42);
  ...
  col *= 0.968 + 0.032 * sin(uv.y * uRes.y * 1.5708 + uTime * 12.0);

  vec3 n = texture2D(tNoise, uv * vec2(3.1, 2.3) + vec2(fract(uTime * 7.0), fract(uTime * 11.0))).rgb;
  col += (n - 0.5) * (0.070 + uHurt * 0.20 + (1.0 - uBattery) * 0.06);
...
export class FX {
  ...
  setSize() { /* internal resolution is fixed on purpose - it is the camcorder sensor */ }
