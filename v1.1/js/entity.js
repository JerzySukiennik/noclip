...
export class Entity {
  ...
  reset() {
    ...
    for (let t = 0; t < 500; t++) {
      ...
      const d = L.distSpawn[cy * GRID_W + cx];
      if (d < 30) continue;
  ...
  update(dt, t, targets, difficulty = 1) {
    ...
    switch (this.state) {
      case STATE.DORMANT:
        if (heard || (los && d < 26)) this.setState(STATE.AWARE);
        else if (this.stateT > 14 && Math.random() < dt * 0.28) this._wander();
        break;
      case STATE.AWARE:
    ...
    if (this.state === STATE.DORMANT) { speed = 0.95; goal = this.wanderGoal; }
    else if (this.state === STATE.AWARE) { speed = 1.9; goal = this.lastSeen; }
  ...
  _wander() {
    const L = this.level;
    for (let t = 0; t < 60; t++) {
      const cx = cxOf(this.pos.x) + Math.floor((Math.random() - 0.5) * 26);
      const cy = cyOf(this.pos.z) + Math.floor((Math.random() - 0.5) * 26);
      if (!L.isOpen(cx, cy)) continue;
      this.wanderGoal = new THREE.Vector3(wx(cx) + CELL / 2, 0, wz(cy) + CELL / 2);
      this.pathT = 0;
      return;
    }
  }
