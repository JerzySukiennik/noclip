...
function fbm(x, y, s, oct, lac, gain) {
  ...
  for (let i = 0; i < oct; i++) {
    sum += amp * vnoise(x * freq, y * freq, s + i * 977);
...
function grain(c, scale, seed, cb, oct = 4) {
  ...
  for (let y = 0; y < c.height; y++) {
    for (let x = 0; x < c.width; x++) {
      const n = fbm(x / scale, y / scale, seed, oct, 2.0, 0.5);
      const rgb = cb(n, x, y);
...
function stains(ctx, w, h, count, seed, col, rmin, rmax, alpha) {
  ...
  for (let i = 0; i < count; i++) {
    const x = hash2(i, 7, seed) * w;
    const y = hash2(i, 19, seed) * h;
...
function wallpaper(h, seed) {
  ...
  // faint vertical weave of the paper
  ctx.globalAlpha = 0.045;
  for (let x = 0; x < W; x += 3) {
    ctx.fillStyle = (x % 6 === 0) ? '#7d7433' : '#d8cf87';
    ctx.fillRect(x, 0, 1, H);
  }
  ctx.globalAlpha = 1;
  stains(ctx, W, H, 9, seed + 3, '108,92,36', 26, 92, 0.34);
  stains(ctx, W, H, 4, seed + 11, '70,60,26', 14, 40, 0.30);

  // seam between paper drops
  ctx.globalAlpha = 0.22; ctx.fillStyle = '#8a8244';
  ctx.fillRect(W - 2, 0, 2, H); ctx.fillRect(0, 0, 1, H);
  ctx.globalAlpha = 1;

  // scuffing where shoes and carts have hit the wall
  const scuffTop = yOf(0.62);
  ctx.globalAlpha = 0.16; ctx.fillStyle = '#4e461f';
  for (let i = 0; i < 44; i++) {
    const x = hash2(i, 5, seed) * W;
    const y = scuffTop + hash2(i, 9, seed) * (H - scuffTop);
    ctx.fillRect(x, y, 3 + hash2(i, 13, seed) * 26, 1 + hash2(i, 17, seed) * 2);
  }
  ctx.globalAlpha = 1;
  ...
  ctx.fillStyle = '#6c6428'; ctx.fillRect(0, H - bbH - 1, W, 1);
  ctx.globalAlpha = 0.35; ctx.fillStyle = '#5a5320';
  ctx.fillRect(0, H - bbH * 0.30, W, bbH * 0.30);
  ctx.globalAlpha = 1;

  // ceiling trim shadow
  ctx.globalAlpha = 0.30; ctx.fillStyle = '#5e5726';
  ctx.fillRect(0, 0, W, Math.max(2, 0.05 * ppm));
  ctx.globalAlpha = 1;
  return c;
}
...
function officeWall(h, seed) {
  ...
  stains(ctx, W, H, 6, seed + 5, '120,120,110', 20, 70, 0.22);
...
function redWall(h, seed) {
  ...
  ctx.globalAlpha = 1;
  stains(ctx, W, H, 7, seed + 2, '52,10,8', 22, 80, 0.42);
  const bbH = 0.11 * ppm;
...
function metalWall(h, seed) {
  ...
  ctx.globalAlpha = 1;
  stains(ctx, W, H, 8, seed + 9, '92,54,20', 16, 60, 0.44);
  const bbH = 0.09 * ppm;
...
function concreteWall(h, seed) {
  ...
  ctx.globalAlpha = 1;
  stains(ctx, W, H, 10, seed + 4, '58,54,44', 20, 80, 0.34);
  return c;
}
...
function tileWall(h, seed) {
  ...
  ctx.globalAlpha = 1;
  stains(ctx, W, H, 6, seed + 6, '96,120,110', 18, 60, 0.30);
  return c;
}
...
function carpet(seed) {
  const c = cv(512, 512);
  const ctx = grain(c, 3.2, seed, (n) => {
    const v = 0.76 + n * 0.36;
    return [Math.min(255, 178 * v), Math.min(255, 158 * v), Math.min(255, 48 * v)];
  }, 3);
  // long low-frequency wear paths
  const img = ctx.getImageData(0, 0, 512, 512), d = img.data;
  for (let y = 0; y < 512; y++) for (let x = 0; x < 512; x++) {
    const n = fbm(x / 90, y / 90, seed + 500, 3, 2, 0.5);
    const k = 0.82 + n * 0.34;
    const i = (y * 512 + x) * 4;
    d[i] *= k; d[i + 1] *= k; d[i + 2] *= k * 0.97;
  }
  ctx.putImageData(img, 0, 0);
  stains(ctx, 512, 512, 14, seed + 21, '92,78,30', 30, 130, 0.36);
  return c;
}
...
function redCarpet(seed) {
  ...
  stains(ctx, 512, 512, 12, seed + 31, '44,8,8', 30, 120, 0.44);
  return c;
}
...
function concreteFloor(seed) {
  ...
  ctx.globalAlpha = 1;
  stains(ctx, 512, 512, 16, seed + 13, '52,48,40', 20, 110, 0.42);
  return c;
}
...
function acoustic(seed) {
  ...
  ctx.globalAlpha = 1;
  stains(ctx, 512, 512, 5, seed + 17, '128,108,54', 26, 90, 0.42);
  return c;
}
