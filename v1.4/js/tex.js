...
function wallpaper(h, seed) {
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    const n = fbm(x / 150, y / 150, seed + 900, 2, 2, 0.5);
  ...
  ctx.globalAlpha = 0.07; ctx.fillStyle = '#6f6528';
  for (let i = 0; i < 30; i++) {
...
function officeWall(h, seed) {
  ...
  stains(ctx, W, H, 5, seed + 5, '120,120,110', 8, 22, 0.12);
...
function carpet(seed) {
  ...
  for (let y = 0; y < 512; y++) for (let x = 0; x < 512; x++) {
    const n = fbm(x / 190, y / 190, seed + 500, 2, 2, 0.5);
    const k = 0.97 + n * 0.06;
    const i = (y * 512 + x) * 4;
    d[i] *= k; d[i + 1] *= k; d[i + 2] *= k;
  }
...
function acoustic(seed) {
  ...
  ctx.globalAlpha = 1;
  stains(ctx, 512, 512, 4, seed + 17, '128,108,54', 10, 30, 0.22);
  return c;
}
