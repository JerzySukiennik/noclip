...
// Level 94: an impossible outdoors. Painted daylight sky and mown grass, matching
// the still Jurek supplied - pale blue up top, warmer toward the horizon.
function skyWall(h, seed) {
  const { c, W, H } = wallBase(h, 512);
  const ctx = c.getContext('2d');
  const g = ctx.createLinearGradient(0, 0, 0, H);
  g.addColorStop(0.00, '#5b9fd6');
  g.addColorStop(0.42, '#8dc2e8');
  g.addColorStop(0.76, '#c3ddf0');
  g.addColorStop(1.00, '#dfeaf2');
  ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
  // a few soft clouds, wrapped so the panorama joins
  ctx.globalAlpha = 0.42;
  for (let k = 0; k < 9; k++) {
    const cx = hash2(k, 3, seed) * W, cy = H * (0.06 + hash2(k, 9, seed) * 0.30);
    const r = 90 + hash2(k, 13, seed) * 150;
    for (const ox of [-W, 0, W]) {
      const rg = ctx.createRadialGradient(cx + ox, cy, 0, cx + ox, cy, r);
      rg.addColorStop(0, 'rgba(255,255,255,.85)');
      rg.addColorStop(1, 'rgba(255,255,255,0)');
      ctx.fillStyle = rg;
      ctx.beginPath(); ctx.ellipse(cx + ox, cy, r, r * 0.30, 0, 0, 6.283); ctx.fill();
    }
  }
  ctx.globalAlpha = 1;
  return c;
...
function grassFloor(seed) {
  const c = cv(512, 512);
  const ctx = grain(c, 2.2, seed, (n) => {
    const v = 0.82 + n * 0.32;
    return [Math.min(255, 108 * v), Math.min(255, 152 * v), Math.min(255, 62 * v)];
  }, 3);
  // blade flecks so it does not read as flat paint
  ctx.globalAlpha = 0.16;
  for (let i = 0; i < 2600; i++) {
    const x = hash2(i, 5, seed) * 512, y = hash2(i, 11, seed) * 512;
    ctx.fillStyle = hash2(i, 17, seed) > 0.5 ? '#8ec46a' : '#4e7a34';
    ctx.fillRect(x, y, 1, 1 + hash2(i, 23, seed) * 2);
  }
  ctx.globalAlpha = 1;
  return c;
}
...
const WALLS = { wallpaper, office: officeWall, red: redWall, metal: metalWall, concrete: concreteWall, tile: tileWall, sky: skyWall };
const FLOORS = { carpet, redcarpet: redCarpet, vinyl, concrete: concreteFloor, tile: (s) => tileWall(2.0, s), grass: grassFloor };
const CEILS = { tile: acoustic, concrete: concreteFloor, metal: metalCeil, sky: (s) => skyWall(6, s) };
