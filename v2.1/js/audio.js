...
export class Audio {
  ...
  kick() {
    if (!this.decks || !this.musicMode || !this.hasMusic) return;
    const d = this.decks[this.deck];
    if (!d.el.paused || !d.el.src) return;
    if (this.current && this.current.start) { try { d.el.currentTime = this.current.start; } catch (e) { } }
    const p = d.el.play();
  ...
  async load(onProgress) {
    ...
    // the two continuous beds are generated, so they never seam
    this.buf.buzz = this.seamlessHum(4.0, [[120, 0.30], [240, 0.15], [360, 0.075], [7000, 0.012]], 0.05);
    this.buf.mains = this.seamlessHum(4.0, [[50, 0.34], [100, 0.14], [150, 0.05]], 0.02);
  ...
  _initPlayer() {
    // Two decks. One element cannot cross-fade with itself, and ducking to silence
    // at the seam is still an audible hole every time the loop comes round.
    this.decks = [0, 1].map(() => {
      ...
      gain.gain.value = 0;
      src.connect(gain);
      gain.connect(this.busMus);
      return { el, gain };
    });
    this.deck = 0;
    const el = this.decks[0].el;
    this.el = el;
    this.elGain = this.decks[0].gain;
    this.queue = [];
    this.queueMode = null;
    this.musicFails = 0;
    this.xfade = 2.2;
    ...
    for (const d of this.decks) {
      d.el.addEventListener('timeupdate', () => {
        if (d !== this.decks[this.deck]) return;
        const e = this.current;
        if (!e) return;
        const end = e.end || d.el.duration || 0;
        if (!end) return;
        const left = end - d.el.currentTime;
        if (left > this.xfade || this._looping) return;
        this._looping = true;
        const list = this._slotList(this.musicMode);
        // start the next pass on the other deck and cross-fade over the seam
        this._crossTo(list.length < 2 ? e : null, left);
      });
      d.el.addEventListener('ended', () => {
        if (d !== this.decks[this.deck]) return;
        this.musicFails = 0;
        if (!this._looping) this._nextTrack();
      });
    }
  ...
  // Bring `e` up on the idle deck while the live one fades out over `secs`.
  _crossTo(e, secs) {
    const entry = e || (this.queue.length ? null : null) || this.current;
    const next = e || this._pickNext();
    if (!next) { this._looping = false; return; }
    const cur = this.decks[this.deck];
    const other = this.decks[1 - this.deck];
    const dur = Math.max(0.4, Math.min(this.xfade, secs || this.xfade));
    this._startDeck(other, next);
    const n = this.ctx.currentTime;
    const vol = this._volFor(this.musicMode);
    other.gain.gain.cancelScheduledValues(n);
    other.gain.gain.setValueAtTime(0.0001, n);
    other.gain.gain.linearRampToValueAtTime(vol, n + dur);
    cur.gain.gain.cancelScheduledValues(n);
    cur.gain.gain.setValueAtTime(Math.max(0.0001, cur.gain.gain.value), n);
    cur.gain.gain.linearRampToValueAtTime(0.0001, n + dur);
    const oldEl = cur.el;
    setTimeout(() => { try { oldEl.pause(); } catch (err) { } }, dur * 1000 + 120);
    this.deck = 1 - this.deck;
    this.el = other.el;
    this.elGain = other.gain;
    this.current = next;
    setTimeout(() => { this._looping = false; }, dur * 1000 + 60);
  }
  ...
  _pickNext() {
    ...
    if (!this.queue.length || this.queueMode !== this.musicMode) {
      this.queue = this._shuffled(list);
      this.queueMode = this.musicMode;
    }
    return this.queue.shift() || list[0];
  }
  ...
  _startDeck(deck, e) {
    ...
    deck.el.load();
    const seek = () => { try { if (e.start) deck.el.currentTime = e.start; } catch (err) { } };
  ...
  _playEntry(e, vol, fade = 2.6) {
    ...
    const n = this.ctx.currentTime;
    other.gain.gain.cancelScheduledValues(n);
    other.gain.gain.linearRampToValueAtTime(0.0001, n + 0.3);
    setTimeout(() => { try { other.el.pause(); } catch (err) { } }, 380);
    this._startDeck(deck, e);
    this.el = deck.el;
    this.elGain = deck.gain;
  ...
  _stopFile(fade) {
    ...
    this._looping = false;
    for (const d of this.decks) {
      const g = d.gain.gain, n = this.ctx.currentTime;
      g.cancelScheduledValues(n);
      g.setValueAtTime(Math.max(0.0001, g.value), n);
      g.linearRampToValueAtTime(0.0001, n + fade);
      const el = d.el;
      setTimeout(() => { try { el.pause(); } catch (e) { } }, fade * 1000 + 100);
    }
  }
  ...
  // A room tone has to be generated, not looped from a recording: any sample short
  // enough to hold in memory seams audibly every time round, and a 1.78 s ballast
  // recording on repeat is the single most irritating thing in the mix.
  seamlessHum(seconds, parts, noiseAmt) {
    const c = this.ctx, sr = c.sampleRate;
    const n = Math.floor(sr * seconds);
    const b = c.createBuffer(1, n, sr);
    const d = b.getChannelData(0);
    for (const [freq, amp] of parts) {
      // snap to a whole number of cycles per buffer so the wrap is phase-continuous
      const cycles = Math.max(1, Math.round(freq * seconds));
      const w = 2 * Math.PI * cycles / n;
      for (let i = 0; i < n; i++) d[i] += Math.sin(w * i) * amp;
    }
    if (noiseAmt) {
      const noise = new Float32Array(n);
      let last = 0;
      for (let i = 0; i < n; i++) { last = (last + 0.06 * (Math.random() * 2 - 1)) / 1.06; noise[i] = last * 6; }
      // cross-fade the tail over the head so the noise wraps without a click
      const x = Math.floor(sr * 0.25);
      for (let i = 0; i < x; i++) {
        const t = i / x;
        noise[i] = noise[i] * t + noise[n - x + i] * (1 - t);
      }
      for (let i = 0; i < n; i++) d[i] += noise[i] * noiseAmt;
    }
    return b;
  }
